export default defineEventHandler(async (event) => {
  ...
  return { configured: { sumopod: Boolean(config.sumopodApiKey), bce: Boolean(config.bceApiUrl && config.bcePartnerKey), rajaongkir: Boolean(config.rajaOngkirApiKey && config.rajaOngkirOriginId), r2: Boolean(bindings(event).PRODUCT_IMAGES) }, failed };
});
