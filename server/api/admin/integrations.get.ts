export default defineEventHandler(async (event) => {
  ...
  return { configured: { sumopod: Boolean(config.sumopodApiKey), bce: Boolean(config.bceApiUrl && config.bcePartnerKey), r2: Boolean(bindings(event).PRODUCT_IMAGES) }, failed };
});
