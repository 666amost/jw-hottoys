...
export default defineEventHandler(async (event) => {
  ...
  if (inserted.meta.changes && (rank[parsed.data.status] ?? -1) >= (rank[shipment.status] ?? -1)) {
    await db.prepare("UPDATE shipments SET status=?,updated_at=?,error_message=? WHERE id=?")
      .bind(parsed.data.status, now, parsed.data.status === "exception" ? parsed.data.note ?? "BCE exception" : null, shipment.id).run();
  }
  if (parsed.data.status === "delivered") await db.batch(deliveredOrderReconciliationStatements(db, shipment.id, now));
