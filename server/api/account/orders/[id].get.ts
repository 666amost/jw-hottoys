export default defineEventHandler(async (event) => {
  ...
  const order = await db.prepare(`SELECT o.*,p.payment_url,p.expires_at payment_expires_at,s.id shipment_id,s.provider shipping_provider,s.awb_number,s.status shipment_status,s.error_message shipment_error,q.service_name shipping_service,q.etd shipping_etd
    FROM orders o LEFT JOIN payments p ON p.order_id=o.id LEFT JOIN shipments s ON s.order_id=o.id LEFT JOIN shipping_quotes q ON q.order_id=o.id
